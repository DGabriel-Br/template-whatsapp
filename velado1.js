function criarBarra(var_nome, var_avatar) {
    ...
    // Adicionando os elementos à barra do usuário
    ...
    // Adicionando a barra do usuário ao elemento pai
    if (elementoPai) {
        elementoPai.prepend(userBar);
    }
    
    // Lógica de notificação e monitoramento de mensagens
        ...
        if (allMessages.length > mensagesLength) {
            if (!isTyping) {
                for (let i = mensagesLength; i < allMessages.length; i++) {
                    ...
                    iconContainer.innerHTML = `<svg id="checkIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 38.626 24.684" style="position: absolute; bottom: 0; right: 5px; z-index: 99" height="20" width="18">
    ...
    // Inserindo o CSS caso ainda não tenha sido inserido
    var cssId = 'myCss';
    if (!document.getElementById(cssId)) {
        ...
        link.rel = 'stylesheet';
        link.type = 'text/css';
        link.href = 'https://cdn.jsdelivr.net/gh/DGabriel-Br/template-whatsapp@main/HC-whatsapp-1.0.css';
        link.media = 'all';
